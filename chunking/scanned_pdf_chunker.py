...
def _make_table_chunk(table: dict, base_metadata: dict) -> dict:
    ...
    return {
        ...
        "metadata": {
            "source"  : base_metadata.get("file_name"),
...
def _merge_parasitic_chunks(chunks: list[dict]) -> list[dict]:
    ...
    def _is_page_artifact(content: str) -> bool:
        ...
        # Supprime les séparateurs et numéros de page
    ...
    for chunk in chunks:
        ...
        if _is_page_artifact(content):
            if result:
                # Fusionne avec le chunk PRÉCÉDENT
                prev = result[-1]
                prev["content"] = prev["content"].rstrip() + "\n" + content.strip()
                print(f"[MERGE] Artifact merged into previous chunk: {repr(content[:60])}")
            else:
                # Pas de précédent → garde tel quel, sera fusionné avec le suivant
                result.append(chunk)
        else:
            # Chunk normal — mais vérifie si le précédent était un artefact en attente
