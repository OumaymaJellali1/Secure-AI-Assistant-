...
Upload flow:
  ...
  4. Stamp document_id + uploaded_by into chunks
  5. embed_chunks() → dense + sparse vectors
...
def _run_ingestion(
    file_path   : str,
    document_id : str,
    user_id     : str,          # ← needed to stamp uploaded_by
) -> tuple[int, str]:
    """
    Run the 3-step pipeline:
      ingest_file → stamp document_id + uploaded_by → embed_chunks → store
    ...
    # Step 2: stamp document_id AND uploaded_by into every chunk's metadata
    for chunk in chunks:
        chunk.setdefault("metadata", {})["document_id"] = document_id
        chunk.setdefault("metadata", {})["uploaded_by"]  = user_id   # ← NEW
...
async def upload_document(
    ...
    try:
        chunks_count, source_type = _run_ingestion(
            str(file_path),
            document_id,
            user_id,            # ← passed through so uploaded_by is stamped
        )
...
def download_document(document_id: str, current_user: dict = Depends(get_current_user)):
    ...
    with engine.connect() as conn:
        row = conn.execute(text("""
            SELECT d.file_path, d.filename FROM documents d
            INNER JOIN document_permissions dp ON dp.document_id = d.document_id
            WHERE d.document_id = :did AND dp.user_id = :uid
        """), {"did": document_id, "uid": user_id}).fetchone()
    if not row: raise HTTPException(404, "Document not found or access denied")
    file_path, filename = row[0], row[1]
    if not file_path or not Path(file_path).exists(): raise HTTPException(404, "File not found on disk")
